...
const svg = d3.select("#vertical_linegraph")
  .append("svg")
...
function generateLine2Graph(csvPath) {
    ...
    d3.csv(csvPath).then(function(data) {
        ...
        // Add Y axis
        y.domain([0, d3.max(data, function(d) { return +d.verticalerror; })])
        svg.append("g")
            .attr("class", "y axis")
            .call(d3.axisLeft(y))
            .style("font-size", "14px");
        ...
        const color = d3.scaleOrdinal()
            .range(['#e41a1d','#377eb9','#4daf4b','#984ea4','#ff7f01','#ffff34','#a65629','#f781cf','#999999'])
        ...
        svg.selectAll("labels") // legend text
            ...
            .append("text")
                .attr("id", (d => d[0]))
                .attr("class", "legend v") // applies a class for styling
        ...
        svg.selectAll(".legend.v")
            .on("click", function(event, d) {
                var opac = d3.selectAll("#" + d[0] + ".line.v").style("opacity");
                if (opac == 1) {
                    opac = 0;
                } else {
                    opac = 1;
                }
                d3.selectAll("#" + d[0] + ".line.v")
                    .transition()
                    .duration(100)
                    .attr("opacity", opac);
            })
            ...
            .join("path")
                ...
                .attr("d", function(d) {
                    return d3.line()
                        ...
                        .y(function(d) { return y(+d.verticalerror); })
                        (d[1])
                })
...
d3.select(".vertical-submit").on("click", function(event, d) {
...
d3.select(".vertical-toggle-brush").on("click", function(event, d) {
    if (brushOn) {
        svg.select(".brush").style("display", "none")
        brushOn = false;
...
function updateGraph(new_csv_path) { // Updates the graph AFTER listening for an event to occur.
    ...
    y = d3.scaleLinear()
        .domain([0, d3.max(new_data, function(d) { return +d.verticalerror; })])
        .range([height, 0]);
...
generateLine2Graph('src/dataServer.csv'); // this is the default display
